...
4.	如何使用
	...
	2.	在对应文件中输入以下内容：中文摘要(abs-chinese)、英文摘要(abs-english)、前言(preface)、第一章(chap1)、第二章(chap2)、结语(epilogue)、参考文献(bib)、附录(appendix)、致谢(acknowledge)等；如有你不需要的部分，注释掉即可；模板中使用了`\lipsum`,`\zhlipsum`生成了随机文本填充正文、使用规范[1]和图书馆模板[2]填充了其他文本，输入你的内容时删掉即可
...
5.	一些个性化设定：
	...
	4.	关于单双页：
		1.	一般来说，书籍的每一个部分（摘要、目录、章等等）应该在右侧（奇数页）开始，Build20200424起支持了这一特性
...
8.	欢迎在致谢中提到我们；
	感谢图书馆工作人员制作的模板，尽管其存在若干问题，仍然给予了我们很多帮助；
...
3.	本模板基于由win平台的Acrobat生成的PDF（`ref/[2]`, `comparison/OWT`）制作，不同平台、软件生成的PDF间可能存在一定的肉眼难以分辨的区别
...
3.	修复了目录的问题，现在每一项的页码的字号与其标题的字号一致了，所有的引导点都是中文省略号（圆点）了
